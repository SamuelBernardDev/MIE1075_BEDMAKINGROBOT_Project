...
class CustomPyBullet:
    ...
    def loadsoftbody(self, body_name: str, **kwargs: Any) -> None:
        """Load URDF file.

        Args:
            body_name (str): The name of the body. Must be unique in the sim.
        """
        self._bodies_idx[body_name] = self.physics_client.loadSoftBody(**kwargs)
    ...
    def lsb(
        ...
        mass: float = 1.0,
        useNeoHookean: int = 0,
        useBendingSprings: int = 1,
        useMassSpring: int = 1,
        springElasticStiffness: float = 40,
        springDampingStiffness: float = 0.1,
        springDampingAllDirections: int = 1,
        useSelfCollision: int = 1,
        frictionCoeff: float = 0.5,
        useFaceContact: int = 1,
        ) -> None:
        """Create a soft body and assign it to the simulation.
        ...
        """
        body_id = self.physics_client.loadSoftBody(
            fileName=file_name,
